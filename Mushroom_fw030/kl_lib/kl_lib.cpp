...
// Disable PLL first!
// HsePreDiv: 1...16; PllMul: pllMul[]
uint8_t Clk_t::SetupPLLDividers(uint8_t HsePreDiv, PllMul_t PllMul, PllSrc_t Src) {
    if(RCC->CR & RCC_CR_PLLON) return FAILURE;    // PLL must be disabled to change dividers
    // Set HSE divider
    HsePreDiv--;
    ...
    // Setup PLL divider
    tmp = RCC->CFGR;
    tmp &= ~(RCC_CFGR_PLLMUL | RCC_CFGR_PLLSRC);
    tmp |= ((uint32_t)PllMul) << 18;
    if(Src == plsHSEdivPREDIV) tmp |= RCC_CFGR_PLLSRC;
    RCC->CFGR = tmp;
    return OK;
}
